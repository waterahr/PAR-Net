...
def max_out(inputs, num_units, axis=None):
    ...
    outputs = tf.reduce_max(inputs, -1, keepdims=True)
    return outputs

def ave_out(inputs, num_units, axis=None):
    shape = inputs.get_shape().as_list()
    if shape[0] is None:
        shape[0] = -1
    if axis is None:  # Assume that channel is the last dimension
    ...
    shape += [num_channels // num_units]
    #print(shape)
    #outputs = tf.reduce_mean(tf.reshape(inputs, shape), -1, keep_dims=False)
    outputs = tf.reduce_mean(inputs, -1, keepdims=True)
    return outputs
